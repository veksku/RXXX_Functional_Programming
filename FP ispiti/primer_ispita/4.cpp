...
{
// POCETAK STUDENTSKOG KODA
    std::vector<std::string> sve_reci = ranges::istream<std::string>(std::cin)
                                       | to<std::vector<std::string>>();
 
    auto bez_ponavljanja = sve_reci | views::unique;
 
    std::copy(ranges::begin(bez_ponavljanja), ranges::end(bez_ponavljanja),
